...
package ruijie.com.my12306.widget.swipeback;
...
public class ViewDragHelper {
   ...
   * and
   * {@link com.github.obsessive.library.swipeback.ViewDragHelper.Callback#onEdgeDragStarted(int,
   * int)}
   ...
   * {@link com.github.obsessive.library.swipeback.ViewDragHelper.Callback#tryCaptureView(View,
   * int)}
  ...
  public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
    ...
    mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(),
        (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
        (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId), minLeft,
      ...
      case MotionEventCompat.ACTION_POINTER_UP: {
        ...
        if (mDragState == STATE_DRAGGING && pointerId == mActivePointerId) {
          ...
          for (int i = 0; i < pointerCount; i++) {
            ...
            final float x = MotionEventCompat.getX(ev, i);
            final float y = MotionEventCompat.getY(ev, i);
            if (findTopChildUnder((int) x, (int) y) == mCapturedView && tryCaptureViewForDrag(
                mCapturedView, id)) {
